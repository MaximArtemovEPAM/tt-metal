...
#include "device/flash_mla_prefill_op.hpp"
#include "device/joint_flash_mla_prefill_op.hpp"
...
std::tuple<ttnn::Tensor, ttnn::Tensor> ExecuteJointFlashMLAPrefill::invoke(
    QueueId queue_id,
    const ttnn::Tensor& input_tensor_q,
    const ttnn::Tensor& input_tensor_k,
    ...
    auto results = tt::tt_metal::operation::run(
        JointFlashMLAPrefill{
            .joint_strategy = joint_strategy,
            .scale = scale,
            .output_mem_config = tt::tt_metal::operation::DEFAULT_OUTPUT_MEMORY_CONFIG,
            .program_config = std::move(program_config),
            .compute_kernel_config = kernel_config_val},
        {input_tensor_q, input_tensor_k, joint_tensor_q, joint_tensor_k},
        {},
        {},
        queue_id);
