...
#include "joint_flash_mla_prefill_op.hpp"

#include "joint_flash_mla_prefill_program_factory.hpp"
...
void JointFlashMLAPrefill::validate(const std::vector<Tensor>& input_tensors) const {
...
std::uint32_t JointFlashMLAPrefill::get_q_chunk_size() const {
    return this->program_config ? this->program_config->q_chunk_size : 32;
}

std::uint32_t JointFlashMLAPrefill::get_k_chunk_size() const {
    return this->program_config ? this->program_config->k_chunk_size : 32;
}

std::vector<TensorSpec> JointFlashMLAPrefill::compute_output_specs(const std::vector<Tensor>& input_tensors) const {
...
operation::ProgramWithCallbacks JointFlashMLAPrefill::create_program(
    ...
    return detail::joint_flash_mla_prefill(
        input_tensor_q,
        input_tensor_k,
        input_tensor_v,
        joint_tensor_q,
