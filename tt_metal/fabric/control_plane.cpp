...
ControlPlane::ControlPlane(const std::string& mesh_graph_desc_file) {
    ...
    TT_FATAL(!((mesh_id_env == -1) ^ (host_rank_id_env == -1)), "TT_METAL_MESH_ID and TT_METAL_HOST_RANK_ID must both be simulteaneously set or unset");

    if (mesh_id_env >= 0) {
        MeshId mesh_id{mesh_id_env};
        HostRankId host_rank{host_rank_id_env};
...
void ControlPlane::assign_direction_to_fabric_eth_core(const FabricNodeId& fabric_node_id, const CoreCoord& eth_core, RoutingDirection direction) {
    auto physical_chip_id = this->logical_mesh_chip_id_to_physical_chip_id_mapping_.at(fabric_node_id);
...
void ControlPlane::assign_intermesh_link_directions_to_local_host(const FabricNodeId& fabric_node_id) {
    ...
    auto physical_chip_id = this->logical_mesh_chip_id_to_physical_chip_id_mapping_.at(fabric_node_id);
    const auto& connected_chips_and_eth_cores =
        tt::tt_metal::MetalContext::instance().get_cluster().get_ethernet_cores_grouped_by_connected_chips(physical_chip_id);

    for (const auto& [connected_mesh_id, edge] : inter_mesh_connectivity[*fabric_node_id.mesh_id][fabric_node_id.chip_id]) {
    ...
    for (const auto& [eth_core, eth_chan] : intermesh_links) {
        ...
        for (const auto& [connected_mesh_id, edge] : inter_mesh_connectivity[*fabric_node_id.mesh_id][fabric_node_id.chip_id]) {
            for (const auto& [candidate_desc, candidate_peer_desc] :
                peer_intermesh_link_tables_[connected_mesh_id]) {
        ...
        router_port_directions_to_physical_eth_chan_map_.at(fabric_node_id)[intermesh_routing_direction].push_back(eth_chan);
    }
}
...
bool ControlPlane::is_local_mesh(MeshId mesh_id) const {
    if (local_mesh_info_.has_value()) {
        return local_mesh_info_->mesh_id == mesh_id;
    }
    return true;
}
